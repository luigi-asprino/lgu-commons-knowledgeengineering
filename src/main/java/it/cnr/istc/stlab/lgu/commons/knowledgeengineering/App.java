...
import it.cnr.istc.stlab.lgu.commons.files.File;
		...
		try {
			...
			if (!new File(entityList).exists()) {
				...
				WalkGenerator.writeEntityListFromQuery(d,
						"SELECT ?r {?r a <http://www.ontologydesignpatterns.org/ont/dul/DUL.owl#Object> } LIMIT 10",
						"r", entityList);
			}

			RDF2Vec.computeSimilarity(config.getString("pythonBin"), config.getString("projectBasePath"), walksFile,
					false, entityList, config.getString("workdir") + "/similarities", config.getInt("dimensionality"),
